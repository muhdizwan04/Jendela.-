...
struct WidgetMacSettings: Codable, Equatable {
...
enum SettingsStore {
    ...
    static func load() -> WidgetMacSettings {
        guard let data = try? Data(contentsOf: fileURL),
              let decoded = try? JSONDecoder().decode(WidgetMacSettings.self, from: data)
        else { return WidgetMacSettings() }
        return decoded
    }

    /// Off the main thread — settings are saved on a debounce, never in a hot path.
    static func save(_ settings: WidgetMacSettings) {
        let url = fileURL
        queue.async {
