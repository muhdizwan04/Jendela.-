...
APP_DIR="$PROJECT_DIR/WidgetMac.app"
...
if [[ ! -d "$PROJECT_DIR/WidgetMac.xcodeproj" ]]; then
  ruby "$PROJECT_DIR/Support/generate_project.rb"
fi

xcodebuild -project "$PROJECT_DIR/WidgetMac.xcodeproj" \
  -scheme WidgetMac -configuration Debug \
  -derivedDataPath "$DERIVED" \
  -allowProvisioningUpdates build

# `open` only activates an already-running instance, so quit first or you keep
# looking at the previous build.
osascript -e 'quit app "WidgetMac"' >/dev/null 2>&1 || true
pkill -x WidgetMac >/dev/null 2>&1 || true

rm -rf "$APP_DIR"
cp -R "$DERIVED/Build/Products/Debug/WidgetMac.app" "$APP_DIR"

# WidgetKit finds the extension through LaunchServices, and a stale record
# pointing at DerivedData will shadow this copy.
"$LSREG" -u "$DERIVED/Build/Products/Debug/WidgetMac.app" >/dev/null 2>&1 || true
"$LSREG" -f -R "$APP_DIR"
